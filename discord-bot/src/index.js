...
client.on('ready', async() => {
    ...
    setInterval(async() => {
        ...
        const embed = new MessageEmbed()
            ...
            .setColor('#4f69c9')
            .setTimestamp()
            .setFooter({ text: `Followers: ${new Intl.NumberFormat().format(post.authorMeta.fans)} | Videos: ${new Intl.NumberFormat().format(post.authorMeta.video)} | Likes: ${new Intl.NumberFormat().format(post.authorMeta.heart)}` });

        tiktokChannel.send({
            content: process.env.NEW_POST.replace(`{url}`,link),
            embeds: [embed]
        });
    }, 15000)
})
